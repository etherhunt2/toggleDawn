...
export default function Chaptershelf() {
    ...
    const fetchChapters = async () => {
      try {
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_API}/chapters${
            bookSlug ? `?bookSlug=${bookSlug}` : ""
          }`
        );
        if (response.data) {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_API}/categories?type=chapter`
        );
        if (response.data) {
  ...
  const filteredChapters =
    selectedCategory === "all"
      ...
      : chapters.filter((chapter) =>
          chapter.category.includes(selectedCategory)
        );
                ...
                description={
                  <div
                    dangerouslySetInnerHTML={{
                      __html: chapter.description.substring(0, 100) + "...",
                    }}
                  />
                }
                type={`simple` as const}
                link={`chapters/${chapter.slug}`}
                images={[chapter.featuredImage]}
              />
            ))}
          </motion.div>
        </div>
